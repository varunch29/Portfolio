...
import { createContext, useContext, useEffect, useId, useImperativeHandle, useRef, useState } from "react";
...
function Map({ children, className, theme: themeProp, styles, projection, viewport, onViewportChange, loading = false, ref, ...props }) {
	...
	}, [mapInstance, resolvedTheme, styles?.dark, styles?.light, clearStyleTimeout]);
...
function MapMarker({
	...
	}, [marker, longitude, latitude, draggable, markerOptions.offset, markerOptions.rotation, markerOptions.rotationAlignment, markerOptions.pitchAlignment]);
	return (
...
function MarkerPopup({ children, className, closeButton = false, ...popupOptions }) {
	...
	return createPortal(<div
		className={cn("bg-popover text-popover-foreground animate-in fade-in-0 zoom-in-95 relative rounded-md border p-3 shadow-md",
			className
		)}>
		{closeButton && (
			<button
...
function MarkerTooltip({ children, className, ...popupOptions }) {
	...
	return createPortal(<div className={cn(
...
function MapPopup({ longitude, latitude, onClose, children, className, closeButton = false, ...popupOptions }) {
	...
	}, [popup, longitude, latitude, popupOptions]);
	const handleClose = () => {
		popup.remove();
	};
	return createPortal(<div className={cn(
...
function MapClusterLayer({
	...
	const callbacksRef = useRef({ onPointClick, onClusterClick });
	useEffect(() => {
	...
	useEffect(() => {
		...
		const handlePointClick = (e) => {
			...
			callbacksRef.current.onPointClick(feature, coordinates);
		};
		...
		const handleMouseEnterPoint = () => {
			if (callbacksRef.current.onPointClick) {
				map.getCanvas().style.cursor = "pointer";
			}
		};
